...
ostream& Book::write( ostream& outBin ) const {
	char temp[128];
	strcpy( temp, isbn.c_str() );
	outBin.write( temp, 18 ); //ISBN are 13 chars+4 dashes+\0
	strcpy( temp, title.c_str() );
	outBin.write( temp, 128 ); // max title, etc. 127 chars
	strcpy( temp, author.c_str() );
	outBin.write( temp, 128 );
	strcpy( temp, publisher.c_str() );
	outBin.write( temp, 128 );
	outBin.write( (char*)&year, sizeof(int) );
	outBin.write( (char*)&edition, sizeof(int) );
return outBin;
}

istream& Book::read( istream& inBin ) {
	char temp[128];
	...
	inBin.read((char*)&year, sizeof(int));
	inBin.read((char*)&edition, sizeof(int));
